          ...
          <div class="ibox">
            ...
            <div class="ibox-content">
                <div class="row text-center">
                    <?php $total = 0; foreach ($materiels as $key => $materiel) {
                        $stock = $materiel->enAgence(Home\PARAMS::DATE_DEFAULT, dateAjoute(1), $agence->id);
                        $prix = $stock;
                        $total += $prix ?>
                        <div class="col-sm-4 col-md-3 border-left border-bottom">
                            <div class="p-xs">
                                ...
                                <h6 class="no-margins text-uppercase gras <?= ($stock > $params->ruptureStock)?"":"clignote" ?>"><?= $materiel->name() ?> </h6>
                            </div>
                        </div>
                    <?php } ?>
                </div>
                        ...
                        <table class="table table-bordered table-hover">
                            <thead>
                                <tr>
                                    ...
                                    <?php foreach ($materiels as $key => $materiel) {  ?>
                                        <th class="text-center"><small class="gras"><?= $materiel->name() ?></small></th>
                                    <?php } ?>
                                </tr>
                            </thead>
                            <tbody>
                                ...
                                while ($index <= $date2) { ?>
                                    <tr>
                                        ...
                                        <?php foreach ($materiels as $key => $materiel) {
                                            $stock = $materiel->enAgence(Home\PARAMS::DATE_DEFAULT, $index, $agence->id);
                                            $production = $materiel->production($index, $index, $agence->id);
                                            $achat = $materiel->achat($index, $index, $agence->id);
                                            $perte = $materiel->perteAutre($index, $index, $agence->id);
                                            ?>
                                            <td class="cursor myPopover text-center"
                                            data-toggle="popover"
                                            data-placement="right"
                                            title="<small><b><?= $materiel->name() ?></b> | <?= datecourt($index) ?></small>"
                                            data-trigger="hover"
                                            data-html="true"
                                            data-content="
