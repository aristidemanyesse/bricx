 ...
 * 
 */
class REGLEMENTFOURNISSEURCHANTIER extends TABLE
{
			...
			if (count($datas) == 1) {
				...
				if (intval($this->montant) > 0) {
					...
					if (count($datas) == 1) {
						$chantier = $datas[0];
						$chantier->actualise();
						if ($chantier->budgetchantier->solde() >= $this->montant) {
	...
	public static function total(string $date1 = "2020-04-01", string $date2, int $chantier_id =null){
		...
		$requette = "SELECT SUM(mouvement.montant) as montant  FROM reglementfournisseurchantier, mouvement WHERE reglementfournisseurchantier.mouvement_id = mouvement.id AND mouvement.typemouvement_id = ? AND reglementfournisseurchantier.valide = 1 AND DATE(reglementfournisseurchantier.created) >= ? AND DATE(reglementfournisseurchantier.created) <= ? $paras ";
