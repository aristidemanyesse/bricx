 ...
 * 
 */
class VERSEMENTCHANTIER extends TABLE
{
	...
	public function enregistre(){
		...
		if (count($datas) == 1) {
			$this->reference = "VER-CHAN/".date('dmY')."-".strtoupper(substr(uniqid(), 5, 6));
			...
			if ($this->modepayement_id != MODEPAYEMENT::PRELEVEMENT_ACOMPTE) {
				if (intval($this->montant) > 0) {
					...
					if (count($datas) == 1) {
						...
						$mouvement->name = "Nouvel apport financier sur le budget du chantier";
						...
						$mouvement->budgetchantier_id  = $chantier->budgetchantier_id;
	...
	public static function total(string $date1 = "2020-04-01", string $date2, int $chantier_id = null){
		$paras = "";
		if ($chantier_id != null) {
			$paras.= "AND chantier_id = $chantier_id ";
		}
		$requette = "SELECT SUM(mouvement.montant) as montant  FROM versementchantier, mouvement WHERE versementchantier.mouvement_id = mouvement.id AND mouvement.typemouvement_id = ? AND versementchantier.valide = 1 AND DATE(versementchantier.created) >= ? AND DATE(versementchantier.created) <= ? $paras";
	...
	public function sentenseCreate(){
		$this->sentense = "Nouveau apport financier sur le chantier ".$this->chantier->name()." d'un montant de $this->montant";
	}
	public function sentenseUpdate(){
		$this->sentense = "Modification des informations du apport financier sur le chantier N°$this->reference ";
	}
	public function sentenseDelete(){
		$this->sentense = "Suppression de l'apport financier sur le chantier N°$this->reference pour ".$this->chantier->name()." d'un montant de $this->montant";
	}
