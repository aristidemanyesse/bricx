...
if ($action == "validerApprovisionnement") {
	...
	if (count($datas) == 1) {
		...
		if (count($ressources) > 0) {
			...
			$approvisionnement = new APPROCHANTIERRESSOURCE();
									...
									if ($data->status) {
										...
										if ($data->status) {
											foreach ($ressources as $key => $value) {
												...
												if (count($datas) == 1) {
													$ressource = $datas[0];
													$lignecommande = new LIGNEAPPROCHANTIERRESSOURCE;
													$lignecommande->approchantierressource_id = $approvisionnement->id;
											...
											if ($modepayement_id != MODEPAYEMENT::PRELEVEMENT_ACOMPTE && $total > 0) {
												...
												$payement->approchantierressource_id = $approvisionnement->id;
												$data = $payement->save();
											}
...
if ($action == "validerAppro") {
	...
	$datas = APPROCHANTIERRESSOURCE::findBy(["id ="=>$id]);
	if (count($datas) > 0) {
		$appro = $datas[0];
		...
		if (count($ressources) > 0) {
			...
			foreach ($tests as $key => $value) {
				...
				$id = $lot[0];
				$qte = end($lot);
				foreach ($appro->ligneapprochantierressources as $key => $lgn) {
