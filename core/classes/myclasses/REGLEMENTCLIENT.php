...
class REGLEMENTCLIENT extends TABLE
{
	...
	public static function total(string $date1 = "2020-04-01", string $date2, int $agence_id = null){
		...
		$requette = "SELECT SUM(mouvement.montant) as montant  FROM reglementclient, mouvement WHERE reglementclient.mouvement_id = mouvement.id AND mouvement.typemouvement_id = ? AND reglementclient.valide = 1 AND DATE(reglementclient.created) >= ? AND DATE(reglementclient.created) <= ? $paras";
	...
	public static function versements(string $date1 = "2020-04-01", string $date2, int $agence_id = null){
		$paras = "";
		if ($agence_id != null) {
			$paras.= "AND agence_id = $agence_id ";
		}
