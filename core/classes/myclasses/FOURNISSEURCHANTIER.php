 ...
 * 
 */
class FOURNISSEURCHANTIER extends AUTH
{
	...
	public function rembourser(int $montant, Array $post){
		...
		if (intval($montant) > 0 ) {
			if ($this->acompte >= intval($montant)) {
				$payement = new OPERATIONCHANTIER();
				$payement->hydrater($post);
				if ($payement->modepayement_id != MODEPAYEMENT::PRELEVEMENT_ACOMPTE) {
	...
	public static function dettes(int $chantier_id = null){
		$total = 0;
		if ($chantier_id != null) {
			foreach (static::findBy(["chantier_id ="=> $chantier_id]) as $key => $client) {
	...
	public function resteAPayer(){
		$total = 0;
		foreach ($this->fourni("approchantierressource", ["etat_id !="=>ETAT::ANNULEE]) as $key => $appro) {
			$total += $appro->reste();	
		}
		foreach ($this->fourni("approchantierproduit", ["etat_id !="=>ETAT::ANNULEE]) as $key => $appro) {
			$total += $appro->reste();	
		}
	...
	public function versements(string $date1 = "2020-04-01", string $date2){
		$datas = $this->fourni("operationchantier", ["DATE(created) >= " => $date1, "DATE(created) <= " => $date2]);
