...
class TRANSFERTSTOCKBOUTIQUE extends TABLE
{
			...
			if (count($datas) == 1) {
				...
				if (count($datas) == 1) {
					...
					if ($this->emballage_id_source != $this->emballage_id_destination){
						if ($this->quantite >= 1){
							if ($produit->enBoutique(PARAMS::DATE_DEFAULT, dateAjoute(1), $this->emballage_id_source, getSession("agence_connecte_id")) >= $this->quantite) {
	...
	public function sentenseCreate(){
		return $this->sentense = "Nouveau transfert de stock à partir de la boutique ".$this->agence->name();
	}
